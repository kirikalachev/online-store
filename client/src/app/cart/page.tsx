...
export default function CartComponent() {
  ...
  const handleRemove = async (productId: string, priceAtTheTime: number) => {
    const data = await removeFromCart(productId, priceAtTheTime);
    const products = await getProductsByIds(data.items.map(i => i.product._id));
    const combined = data.items.map(item => ({
      ...item,
      product: products.find(p => p._id === item.product._id)!
            ...
            {cartItems.map(item => (
              ...
              >
                <div className="flex items-center gap-4">
                  <img src={`http://localhost:3000${item.product.mainImage}`} alt={item.product.name} className="w-16 h-16 object-cover rounded-lg" />
                  <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => handleRemove(item.product._id, item.price)}
